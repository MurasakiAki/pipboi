...
def tell_location(username):
    ...
    try:
        ...
        system_logger.log_message("ERROR", "File locations.json missing/not found.")
        data = {'locations': []}
    ...
    for location in data.get("locations", []):
        if location.get("name") == name:
            ...
            if latitude is not None and longitude is not None:
                system_logger.log_message("INFO", "Telling location based on name.")
                return f"Location {name} is at {latitude}/{longitude}"
                break
            else:
                system_logger.log_message("WARNING", f"Location '{name}' has incomplete data.")
...
def add_location(username, name, latitude, longitude):
    try:
        ...
        system_logger.log_message("WARNING", "File locations.json missing/not found.")
        data = {'locations': []}
...
def remove_location(username, name):
    ...
    with open(f".{username}/.locations.json", 'w') as file:
        json.dump(data, file, indent=4)
        system_logger.log_message("INFO", f"Location '{name}' removed successfully.")
...
def distance(lat1, lng1, lat2, lng2):
    ...
    system_logger.log_message("INFO", f"Telling distance between {lat1}/{lng1} and {lat2}/{lng2}.")
    ...
    for location in data.get("locations", []):
        if location.get("name") == name:
            ...
            if latitude is not None and longitude is not None:
                ...
                dist = geodesic(loc1, loc2).kilometers
                system_logger.log_message("INFO", f"Telling distance between {name} and {lat2}/{lng2}.")
...
def distnames(username, name1, name2):
    ...
    for location in data.get("locations", []):
        ...
        if lat1 is not None and lng1 is not None and lat2 is not None and lng2 is not None:
            ...
            dist = geodesic(loc1, loc2).kilometers
            system_logger.log_message("INFO", f"Distance between {name1} and {name2} is")
...
def disthere(username, input):
    ...
    system_logger.log_message("INFO", f"Telling distance from here to {input}.")
