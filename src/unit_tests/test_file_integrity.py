...
def test_file_integrity():
    sens_config_file_path = os.path.join(script_directory, '../conf/sens_conf.json')
    help_file_path = os.path.join(script_directory, '../.help.txt')
    if not check_file(sens_config_file_path):
        config = {
            "usb_port": "/dev/ttyACM0"
        }
        with open(sens_config_file_path, 'w') as f:
